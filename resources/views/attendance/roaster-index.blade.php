                            ...
                            <div class="row">
                                <div class="col-lg-4 col-md-4 col-sm-4 col-xs-12">
                                    ...
                                    <div class="review-content-section">
                                        <form action="{{ route('leave.roaster-save') }}" method="post" class="dropzone dropzone-custom needsclick add-professors" id="demo1-upload">
                                            @csrf
                                            @if (isset($editable))
                                                ...
                                                @foreach ($days as $day)
                                                    <div class="col-lg-4 col-md-4 col-sm-4 col-xs-4">
                                                        <div class="form-group">
                                                            ...
                                                            <select name="{{ strtolower($day) }}" id="{{ strtolower($day) }}" class="form-control" required>
                                            ...
                                            <div class="static-table-list">
                                                <table class="table">
                                                    ...
                                                    <tbody>
                                                        ...
                                                        <form action="{{route('leave.roster-change',Crypt::encrypt($usr->id))}}" method="post">
                                                            @csrf
                                                            <tr>
                                                                <td>{{++$key}}</td>
                                                                <td>{{$usr->name}}</td>
                                                                <td>{{$usr->emp_code}}</td>
                                                                <td>
                                                                    <select name="assigned_roster" id="assigned_roster" class="form-control">
                                                                        <option value="">Office Duty</option>
                                                                        @foreach ($roaster as $rst)
                                                                            <option value="{{$rst->id}}" {{$usr->roster == $rst->id?'selected':''}}>{{$rst->name}}</option>
                                                                        @endforeach
                                                                    </select>
                                                                </td>
                                                                <td><input type="submit" class="btn btn-primary" value="Change"></td>
                                                            </tr>
                                                        </form>
                                                        @endforeach
                                                    </tbody>
